...
METHODS = c(P_VAL_STR, DISTANCE_STR, INTERVAL_STR, MESP_STR)
...
plot_success_rates = function(correct_decisions_h0_true, correct_decisions_h0_false) {
  ...
  legend("bottomleft",
         inset=0.05,
         legend=c("P-Value", "Distance-Only", MESP_STR, "Interval-Based"),
         col=c("#14a6de", "#f6bb20", "#5b2c7f", "#527f38"),
         cex=0.8,
         pch=19
  )
  lines(x=c(1, 2, 3),
...
col_names = c("h0_point", "alpha", "true_pop_mean", "true_pop_sigma", "sample_n", "MPSD",
  ...
  "decision_interval_based", "decision_distance_only", "sample_ci_lo", "sample_ci_hi", "nominal_power")
...
for(i in 1:n_iter) {
  ...
  sim = rbind(sim, c(null_mean, alpha, mu, sigma, n, MPSD, sample_mean, p_value, decision_perfect_information, decision_p_value,
                        decision_MESP, decision_interval_based, decision_distance_only, 
...
barplot(
  ...
  names.arg=c("P-value", "Distance only", MESP_STR, "Interval based"),
  ylim=c(0, 1),
  ylab="Proportion in %",
